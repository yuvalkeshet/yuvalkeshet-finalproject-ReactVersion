...
class Inbox extends Component {
    ...
    componentDidMount() {
        ...
        fetch(domain + 'api/messages/trash?email=' + JSON.parse(localStorage['userData']).Email)
        ...
        if (this.state.messages != null && this.state.messages.length > 0) {
            ...
            return (
                <MDBContainer style={{ padding: '0px' }}>
                    <h5 style={{ padding: '1.25rem' }}>Trash</h5>
                    <MDBCard>
                        <MDBListGroup id='messages'>
                            {
                                this.state.messages.map((msg) => {
                                    return <MDBListGroupItem href={"/messages/trash/" + msg.MessageCode + '/' + msg.ToUser} style={{ color: "#000" }}>
                                        <div className="d-flex w-100 justify-content-between">
                                            <h6 className="mb-1"><EllipsisText text={'From: ' + msg.FromUser} length={"15"} /></h6>
